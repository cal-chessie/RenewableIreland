...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[Lead Captured]', {
      ref: leadRef, name, email, phone,
      county: county || 'Not provided',
      monthly_bill: monthly_bill ?? null,
      estimate_data: estimate_data ?? null,
      source,
    ...
    try {
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        await supabase.from('leads').insert({
          ...
          monthly_bill: monthly_bill ?? null,
          estimate_data: estimate_data ?? null,
          source, reference: leadRef,
        });
      }
    } catch (dbErr) {
      console.error('[Lead] Supabase save failed (non-fatal):', dbErr?.message || dbErr);
    }
