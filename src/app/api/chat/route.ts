...
async function openaiChat(
  ...
  if (!apiKey) {
    console.error('[SolarChat] OPENAI_API_KEY is not set');
    return null;
  }
  ...
  if (!res.ok) {
    console.error('[SolarChat] OpenAI error', res.status, await res.text());
    return null;
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    /* Call LLM */
    const reply =
      (await openaiChat(apiMessages)) ??
      "I'm sorry, I couldn't generate a response. Please try again or call us at +353 87 395 8424.";
